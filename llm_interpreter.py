        ...
        for attempt in range(max_retries):
            try:
                ...
                if len(directives) == len(notes):
                    return validate_and_sanitize_directives(directives, len(notes), battery)
            except Exception as e:
                err_str = str(e)
                if "RESOURCE_EXHAUSTED" in err_str or "422" in err_str or "429" in err_str:
                    time.sleep(1.0 * (attempt + 1))
                    continue
                else:
                    print(f"Gemini API call error ({e}), falling back to deterministic extractor.")
                    break

    return fallback_interpret_operator_notes(notes, battery)

def validate_and_sanitize_directives(directives: List[DirectiveInterpretation], expected_count: int, battery: Optional[BatteryLimits] = None) -> List[DirectiveInterpretation]:
    sanitized = []
    ...
    for i in range(expected_count):
        ...
        # Guardrail 1: Check directive type validity and no_op semantics
        if not d or d.directive_type not in allowed_types or d.directive_type == "no_op" or not d.applies or not d.structured_adjustment:
        ...
        # Guardrail 2: Hours must be unique integers 0..23 in ascending order
        raw_hours = adj.hours or []
        clean_hours = sorted(list(set([h for h in raw_hours if isinstance(h, int) and 0 <= h <= 23])))
        if not clean_hours:
            sanitized.append(DirectiveInterpretation(
                note_index=i, applies=False, directive_type="no_op", structured_adjustment=None, explanation="Invalid hour window; defaulted to no_op."
            ))
            continue

        # Guardrail 3: Validate numeric parameters based on directive type
        clean_factor = None
        clean_min_reserve = None
        clean_max_grid = None

        if d.directive_type == "solar_reduction":
            factor = adj.factor if adj.factor is not None else 1.0
            clean_factor = max(0.0, min(1.0, round(float(factor), 4)))
        elif d.directive_type == "minimum_battery_reserve":
            min_val = adj.minimum_energy_kwh if adj.minimum_energy_kwh is not None else 0.0
            max_cap = battery.capacity_kwh if battery else float('inf')
            clean_min_reserve = max(0.0, min(max_cap, round(float(min_val), 2)))
        elif d.directive_type == "max_grid_window":
            grid_val = adj.max_grid_kwh if adj.max_grid_kwh is not None else 0.0
            clean_max_grid = max(0.0, round(float(grid_val), 2))

        clean_adj = StructuredAdjustment(
            hours=clean_hours,
            factor=clean_factor,
            minimum_energy_kwh=clean_min_reserve,
            max_grid_kwh=clean_max_grid
        )
